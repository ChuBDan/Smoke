...
const SmokingStatusForm = () => {
  ...
  const isPaidMember = !!memberPackage && memberPackage.packageName === "VIP";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const smokingLogResponse = await smokingCessationApi.createSmokingLog(
        userId,
        smokingLogData,
        token
      );
      ...
      if (isPaidMember && smokingLogId) {
        try {
          const planResponse = await smokingCessationApi.createPlan(
            userId,
            smokingLogId,
            token
          );
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 text-white p-4 rounded-md hover:bg-blue-700 transition duration-200 disabled:opacity-50 text-lg font-semibold"
          >
