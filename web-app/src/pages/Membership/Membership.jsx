...
const memberId = localStorage.getItem("userId");
  ...
  useEffect(() => {
    axios.get('/user/get-all-membership-packages')
      .then((res) => {
  ...
  try {
    const res = await axios.post(`/user/buy-membership-package/${packageId}/member/${memberId}`);
    ...
    <div className="pb-3 mt-12 font-medium text-zinc-800" style={{ maxWidth: '100%', margin: '0 auto', padding: '60px 20px', minHeight: '100vh' }}>
      <div className="flex flex-col gap-6 items-center">
        <div className="membership-container" style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
          ...
          {packages.map((pkg) => (
            <div key={pkg.id} className="plan-card" style={{ background: '#fff', color: '#333', borderRadius: '10px', width: '350px', padding: '40px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', position: 'relative' }}>
              ...
              <p className="price text-4xl font-bold mt-2">${pkg.price} <span style={{ fontSize: '20px' }}>VND</span></p>
              <p className="text-sm mt-2" style={{ maxHeight: '60px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{pkg.description}</p>
              <button
                style={{
